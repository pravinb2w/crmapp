...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $deal_stages = DealStage::where('status', 1)->get();
        ...
        $deal_progress = $this->deal_progress();
        ...
        $params['deal_progress']    = $deal_progress;
        $params['deal_stages']      = $deal_stages;
    ...
    public function deal_progress()
    {
        $deal_stages = $_POST['deal_stages'] ?? '';
        ...
        if (isset($months) && !empty($months)) {
            foreach ($months as $key => $value) {
                ...
                if (!empty($deal_stages)) {
                    $prog = DealPipline::where('stage_id', $deal_stages)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->count();
                } else {
                    $prog = DealPipline::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->count();
                }
                // $prog = DealPipline::whereNotNull('completed_at')->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->count();

                $xvalues[] = $prog;
            }
        }
        $arr = array('month' => $month, 'yvalues' => $xvalues);
        return $arr;
    }
