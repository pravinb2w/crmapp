        ...
        <div class="modal-body d-flex justify-content-center h-100 p-3" style="width: 790px">
            <div class="w-100">
                ...
                <div>
                    ...
                    <div class="mb-1">
                        <label for="task_name" class="col-form-label">Type of Activity <span
                            ...
                            <select name="activity_type" id="activity_type" class="form-control">
                                ...
                                @if (isset($workflow_type) && !empty($workflow_type))
                                    @foreach ($workflow_type as $item)
                                        <option value="{{ $item }}"
                                            @if (isset($info->activity_type) && $info->activity_type == $item) selected @endif>{{ $item }}
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                    ...
                    <div class="row mb-1">
                        <div class="col-4">
                            ...
                            <div>
                                <div>
                                    @php
                                        $checked = '';
                                        if (isset($info->is_mail_to_customer) && $info->is_mail_to_customer == '1') {
                                            $checked = 'checked';
                                        } elseif (isset($info->is_mail_to_customer) && $info->is_mail_to_customer == '0') {
                                            $checked = '';
                                        } else {
                                            $checked = 'checked';
                                        }
                                    ...
                                    @endphp
                                    <input type="checkbox" name="is_mail_to_customer" id="is_mail_to_customer"
                        ...
                        <div class="col-4">
                            ...
                            <div>
                                <div>
                                    <input type="checkbox" name="is_mail_to_team" id="is_mail_to_team"
                                        {{ isset($info->is_mail_to_team) && $info->is_mail_to_team == '1' ? 'checked' : '' }}
                                        data-switch="success" />
                        ...
                        <div class="col-4">
                            ...
                            <div>
                                <div>
                                    <input type="checkbox" name="is_notification_to_team" id="is_notification_to_team"
                                        {{ isset($info->is_notification_to_team) && $info->is_notification_to_team == '1' ? 'checked' : '' }}
                                        data-switch="success" />
                    ...
                    <div class="mb-1">
                        ...
                        <!-- Success Switch-->
                        <div>
                            <input type="checkbox" name="status" id="switch3"
                                {{ isset($info->status) && $info->status == '1' ? 'checked' : (isset($info->status) && $info->status == '0' ? '' : 'checked') }}
                                data-switch="success" />
...
<script>
    ...
    template_dropdown +=
        '<label for="task_name" class="col-form-label">Email Template <span class="text-danger">*</span></label>';
    template_dropdown +=
        '<div><select name="team_template_id" id="team_template_id" class="form-control"><option value="">--select--</option>';
    if (template.length > 0) {
        for (let index = 0; index < template.length; index++) {
            ...
            template_dropdown += '<option value="' + template[index].id + '" ' + team_selected + '>' + template[index]
                .title + '</option>';
    ...
    cust_template_dropdown +=
        '<label for="task_name" class="col-form-label">Email Template <span class="text-danger">*</span></label>';
    cust_template_dropdown +=
        '<div><select name="customer_template_id" id="customer_template_id" class="form-control"><option value="">--select--</option>';
    if (template.length > 0) {
        for (let index = 0; index < template.length; index++) {
            let selected = '';
            ...
            cust_template_dropdown += '<option value="' + template[index].id + '" ' + selected + '>' + template[index]
                .title + '</option>';
    ...
    notification +=
        '<div><input type="text" name="notification_title" id="notification_title" class="form-control" value="' +
        notification_title + '" required></div>';
    notification += '</div><div class="">';
    notification += '<label for="notification_message" class="col-form-label"> Notification Message </label>';
    notification +=
        '<div><textarea name="notification_message" class="form-control" id="notification_message" cols="30" rows="2" required>' +
        notification_message + '</textarea></div>';
    notification += '</div>';
