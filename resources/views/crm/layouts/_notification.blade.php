...
<div class="p-2" >
    ...
    @forelse ($list as $item)
        <a href="@if( superadmin() && isset($item->url) && !empty($item->url) ) {{ $item->url }} @else javascript:void(0); @endif" class="new-item notify-item @if($item->is_read == 0) unread @endif" 
            @if( auth()->user()->role_id ) 
            onclick="return make_read('{{ $item->id }}')" @endif>
