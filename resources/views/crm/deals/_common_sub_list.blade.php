                            ...
                            <td class="w-75">
                                <div>
                                    <div>{{ strtoupper($litem['activity_type']) }}
                                        ...
                                        @if (isset($litem['pending_at']) &&
                                            !empty($litem['pending_at']) &&
                                            $litem['approved_at'] == null &&
                                            $litem['activity_type'] == 'invoice')
                                        ...
                                        @if (isset($litem['document']) && !empty($litem['document']))
                                            @if ($litem['activity_type'] != 'invoice')
                                                <small class="ml-3">
                                                    <a href="{{ asset('storage/' . $litem['document']) }}"
                                                        target="_blank"> View Files </a>
                                                </small>
                                            @endif
                                        @endif
                                    </div>
                                    <p>
                                        @if ($litem['activity_type'] != 'Notes')
                                            {{ strtoupper($litem['activity_type']) }} -
                                        @endif {{ $litem['subject'] }}
                                    </p>
                            ...
                            <td style="position: relative">
                                ...
                                <div>
                                    @if (isset($litem['done_by']) && !empty($litem['done_by']))
                                        <span class="badge badge-success-lighten"> CompletedBy :
                                            {{ $litem['done_by']->name ?? '' }}</span>
                                    @else
                                        <span class="badge badge-success-lighten"> AddedBy :
                                            {{ $litem['added']->name ?? '' }}</span>
                                    @endif
                                </div>
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                        @if (Auth::user()->hasAccess('deals', 'is_edit') || Auth::user()->hasAccess('deals', 'is_view') || superadmin())
                                            @if ($litem['activity_type'] == 'invoice')
                                                @if (Auth::user()->hasAccess('deals', 'is_edit') || superadmin())
                                                    @if ($litem['pending_at'] == null)
                                                        <a class="dropdown-item" href="#"
                                                            onclick="submit_approve_invoice('{{ $deal_id }}','{{ $litem['id'] }}', 'done')">
                                                            Submit for Approval
                                                        </a>
                                                    @endif
                                                @endif
                                                @if (Auth::user()->hasAccess('deals', 'is_edit') || superadmin())
                                                    <a class="dropdown-item" href="#"
                                                        onclick="unlink_invoice('{{ $deal_id }}','{{ $litem['id'] }}', 'done')">Unlink
                                                        from Deal</a>
                                                @endif
                                            ...
                                            @elseif(($litem['activity_type'] != 'note' &&
                                                $litem['activity_type'] != 'files' &&
                                                Auth::id() == $litem['user_id']) ||
                                                (Auth::id() == $litem['added_by'] && $litem['activity_type'] != 'note' && $litem['activity_type'] != 'files') ||
                                                (superadmin() && $litem['activity_type'] != 'note' && $litem['activity_type'] != 'files'))
                                                <a class="dropdown-item" href="javascript:;"
                                                    onclick="edit_activity('history','{{ $litem['id'] }}', '','{{ $deal_id }}')">Edit
                                                </a>
                                                <a class="dropdown-item" href="javascript:;"
                                                    onclick="activity_comments('{{ $litem['id'] }}')"> Comments </a>
                                                @if (!isset($litem['done_by']))
                                                    <a class="dropdown-item" href="javascript:;"
                                                        onclick="mark_as_done('{{ $litem['id'] }}', '{{ $deal_id }}', 'deal')">Mark
                                                        as Done</a>
                                                @endif
                                            @endif
                                        @endif
                                        @if ((Auth::user()->hasAccess('deals', 'is_delete') && Auth::id() == $litem['user_id']) ||
                                            Auth::id() == $litem['added_by'] ||
                                            superadmin())
                                            @if ($litem['activity_type'] != 'invoice')
                                                <a class="dropdown-item" href="#"
                                                    onclick="change_activity_status('{{ $deal_id ?? '' }}','{{ $litem['id'] ?? '' }}', '{{ strtolower($litem['activity_type']) ?? '' }}' )">Delete</a>
                                            @endif
                                        @else
